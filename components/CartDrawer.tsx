...
const CartDrawer: React.FC<CartDrawerProps> = ({ isOpen, onClose, items, onRemoveItem, onCheckout }) => {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 bg-[#2C2A26]/30 backdrop-blur-sm z-[60] transition-opacity duration-500 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
        onClick={onClose}
      />
      ...
      {/* Drawer */}
      <div
        className={`fixed inset-y-0 right-0 w-full md:w-[450px] bg-[#F5F2EB] z-[70] shadow-2xl transform transition-transform duration-500 ease-in-out border-l border-[#D6D1C7] flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-[#D6D1C7]">
          <h2 className="text-xl font-serif text-[#2C2A26]">장바구니 ({items.length})</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-60">
              ...
              <p className="font-light text-[#5D5A53]">장바구니가 비어 있습니다.</p>
            </div>
          ) : (
            items.map((item, idx) => (
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-sm font-light text-[#2C2A26]">₩{item.price.toLocaleString()}</span>
                    </div>
        ...
        <div className="p-6 border-t border-[#D6D1C7] bg-[#EBE7DE]/30">
          <div className="flex justify-between items-center mb-6">
            <span className="text-sm font-medium uppercase tracking-widest text-[#5D5A53]">소계</span>
            <span className="text-xl font-serif text-[#2C2A26]">₩{total.toLocaleString()}</span>
          </div>
          <p className="text-xs text-[#A8A29E] mb-6 text-center">배송비와 세금은 결제 시 계산됩니다.</p>
          <button
            onClick={onCheckout}
